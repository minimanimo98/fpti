...
function calcScore(allAnswers: Record<number, number>[]) {
  ...
  for (let i = 1; i <= 28; i++) {
    const vals = allAnswers.map((a: Record<number, number>) => a[i]).filter((v: number) => v !== undefined)
    if (vals.length > 0) avgPerQ[i] = vals.reduce((a: number, b: number) => a + b, 0) / vals.length
  }
...
export async function GET(
  ...
  const allAnswers = responses.map((r: { answers: Record<number, number> }) => r.answers)
